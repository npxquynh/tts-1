...
class Tfidf():
    ...
    def compute_Qw(self):
        """
        This one is different from the one in tf.idf
        """
        for (query_id, query) in self.queries.iteritems():
            self.Qw[query_id] = self.compute_tf(query)
            for values in self.Qw.values():
                for token, weight in values:
                    try:
                        # pdb.set_trace()
                        self.Qw[query_id] = weight * self.idf[token]
                    except KeyError:
                        self.Qw[query_id] = 0
    ...
    def score_query_document(self):
        ...
        # Step 0: prepare all the relevant term-frequency & inverse document frequency
        self.compute_tf_idf()
    ...
    def compute_weighted_cosine(self):
        print "Running: score_query_document()"
        t = time.time()
        ...
        # Step 1:# Computing the weighted cosine from the weight of documents & queries
        self.scoreQD = []
        scoreQD = []
        ...
        for (query_id, query) in self.queries.iteritems():
            for (doc_id, doc) in self.docs.iteritems():
                ...
                for token in common_words:
                    nominator += self.Qw[token] * self.Dw[doc_id][word]

                denominator = utility.calculate_square(self.Qw) * utility.calculate_square(self.Dw[doc_id])
                try:
